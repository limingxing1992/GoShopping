...
- (void)setSelectedIndex:(NSInteger)selectedIndex
{
    ...
    JYSegmentBarItem *item  = (id)[self.segmentBar cellForItemAtIndexPath:indexPath];
    item.titleLabel.textColor = [UIColor blackColor];
    
    _selectedIndex = selectedIndex;
    
    NSIndexPath *indexPath_old = [NSIndexPath indexPathForRow:_selectedIndex inSection:0];
    JYSegmentBarItem *item_old  = (id)[self.segmentBar cellForItemAtIndexPath:indexPath_old];
    item_old.titleLabel.textColor = [UIColor redColor];
                  ...
                  cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    segmentBarItem.titleLabel.text = self.names[indexPath.row];
    
    //首页第一次默认为红色
    if (indexPath.row == 0) {
        segmentBarItem.titleLabel.textColor = [UIColor redColor];
    }
